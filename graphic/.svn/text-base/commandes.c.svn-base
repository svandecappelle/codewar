...
void move(processeur liste_processeur[][16], adresse *ad){
 ...
 /*  printf("mot[PC] : %04X\n",*(unsigned short int *)&p->ram[(p->r[PC])%256]); */
/*   printf("mot[P+1] : %04X\n",*(unsigned short int *)&p->ram[(p->r[PC]+2)%256]); */
...
void pop( processeur liste_processeur[][16], adresse *ad){
  ...
  /*récupération de la valeur des opérande*/
    source = (unsigned short int )p->ram[p->r[SP]];
...
void rte( processeur liste_processeur[][16], adresse *ad){
  ...
  if(p->r[SP]<251){
    ...
    p->r[RE] = (unsigned short int )p->ram[p->r[SP]];
    p->r[SP]+=2;
    p->r[PC] = (unsigned short int )p->ram[p->r[SP]];
    p->r[SP]+=2;
  }
  else{
