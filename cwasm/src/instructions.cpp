...
CInstruction::eOperandType CInstruction::tokenToOperandCode(const QString &token, unsigned short &code)
{
   switch (token[0].toAscii())
   {
	...
	operand_label:
	 ...
	 else if (!(opcode() >= JCC && opcode() <= JSR) && opcode() != MOVE)
	 {
...
QByteArray COneOperandInstruction::fullOpCode(QStringList &operands)
{
   ...
   code.resize(2);
   unsigned short *data = (unsigned short *) code.data();
   qDebug() << QString("Instruction %1: opcode: %2 type: %3 opval: %4").arg(name()).arg(opcode()).arg(type).arg(opval,0,16);
   data[0] = opcode() << 11 | type << 8 | ((opval) & 0xFF);
   return code;
}
...
#define NEW_INST2(text,inst) if (up_name == #text)	\
   { \
      CInstruction *i = new C##inst(); \
      i->opcode() = (inst);	     \
      return i;			     \
   }

CInstruction *CInstructionFactory::create(const QString &name)
{
...
void CInstruction::listInstructions()
{
